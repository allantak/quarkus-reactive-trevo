...
public class OrderItemResource {
    ...
    public Uni<Response> create(OrderItem orderItem) {
        return Panache.<OrderItem>withTransaction(orderItem::persist)
                .onItem().transform(inserted -> Response.created(URI.create("/product/" + inserted.getClientName())).build())
                .onItem().invoke(() -> sendMovieToKafka(orderItem));
    }
